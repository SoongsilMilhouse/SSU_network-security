...
int encode_ImgSend(IMG_SEND *src, unsigned char **dst)
{
	...
	*dst = (unsigned char *)calloc(1, sizeof(int) + src->imgLength);
	...
	imgLength = htonl(src->imgLength);
	memcpy(pt, &imgLength, sizeof(int));
	pt += sizeof(int);
...
int decode_ImgSend(unsigned char *src, IMG_SEND **dst)
{
	...
	*dst = (IMG_SEND *)calloc(1, sizeof(IMG_SEND));
	...
	memcpy(&imgLength, pt, sizeof(int));
	(*dst)->imgLength = ntohl(imgLength);
	pt += sizeof(int);
...
int encode_packet(int msgType, void *msg, unsigned char **dst)
{
	...
	case MT_LOGIN_ACK:
		...
		break;
	case MT_IMG_SEND:
		msgLen = encode_ImgSend((IMG_SEND *)msg, &msgBuf); //msg encofing and get msg length
		header = (PACKET_HEADER *)calloc(1, sizeof(PACKET_HEADER)); // make header
		header->msgType = MT_IMG_SEND;
		header->length = msgLen;
		headLen = encode_PacketHeader(header, &headBuf); //encofing header and get header length
		break;
	case MT_IMG_ACK:
		msgLen = encode_ImgAck((IMG_ACK *)msg, &msgBuf); //msg encofing and get msg length
		header = (PACKET_HEADER *)calloc(1, sizeof(PACKET_HEADER)); // make header
